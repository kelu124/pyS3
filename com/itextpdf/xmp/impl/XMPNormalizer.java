...
public class XMPNormalizer {
            ...
            if (arrayForm != null) {
                if (currProp.getOptions().isSimple()) {
                    ...
                    if (arrayForm.isArrayAltText() && !currProp.getOptions().getHasLanguage()) {
                        currProp.addQualifier(new XMPNode(XMPConst.XML_LANG, "x-default", null));
                    }
                } else {
    ...
    private static void transplantArrayItemAlias(Iterator propertyIt, XMPNode childNode, XMPNode baseArray) throws XMPException {
        if (baseArray.getOptions().isArrayAltText()) {
            ...
            childNode.addQualifier(new XMPNode(XMPConst.XML_LANG, "x-default", null));
        }
        propertyIt.remove();
    ...
    private static void migrateAudioCopyright(XMPMeta xmp, XMPNode dmCopyright) {
        try {
            ...
            if (dcRightsArray == null || !dcRightsArray.hasChildren()) {
                ...
                xmp.setLocalizedText("http://purl.org/dc/elements/1.1/", DublinCoreProperties.RIGHTS, "", "x-default", dmValue, null);
            } else {
                int xdIndex = XMPNodeUtils.lookupLanguageItem(dcRightsArray, "x-default");
                if (xdIndex < 0) {
                    XMPMeta xMPMeta = xmp;
                    xMPMeta.setLocalizedText("http://purl.org/dc/elements/1.1/", DublinCoreProperties.RIGHTS, "", "x-default", dcRightsArray.getChild(1).getValue(), null);
