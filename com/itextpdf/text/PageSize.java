...
public class PageSize {
    ...
    public static Rectangle getRectangle(String name) {
        ...
        if (pos == -1) {
            try {
                return (Rectangle) PageSize.class.getDeclaredField(name.toUpperCase()).get(null);
            } catch (Exception e) {
                throw new RuntimeException(MessageLocalization.getComposedMessage("can.t.find.page.size.1", name));
            }
        }
        try {
            return new Rectangle(Float.parseFloat(name.substring(0, pos)), Float.parseFloat(name.substring(pos + 1)));
        } catch (Exception e2) {
            throw new RuntimeException(MessageLocalization.getComposedMessage("1.is.not.a.valid.page.size.format.2", name, e2.getMessage()));
        }
    }
}
