...
public class FinderPatternFinder {
    ...
    private static float centerFromEnd(int[] stateCount, int end) {
        return ((float) ((end - stateCount[4]) - stateCount[3])) - (((float) stateCount[2]) / BaseField.BORDER_WIDTH_MEDIUM);
    }
    ...
    protected static boolean foundPatternCross(int[] stateCount) {
        ...
        float maxVariance = moduleSize / BaseField.BORDER_WIDTH_MEDIUM;
        if (Math.abs(moduleSize - ((float) stateCount[0])) >= maxVariance || Math.abs(moduleSize - ((float) stateCount[1])) >= maxVariance || Math.abs((BaseField.BORDER_WIDTH_THICK * moduleSize) - ((float) stateCount[2])) >= BaseField.BORDER_WIDTH_THICK * maxVariance || Math.abs(moduleSize - ((float) stateCount[3])) >= maxVariance || Math.abs(moduleSize - ((float) stateCount[4])) >= maxVariance) {
            return false;
        }
        return true;
    }
