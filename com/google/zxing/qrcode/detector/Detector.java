...
public class Detector {
    ...
    protected final DetectorResult processFinderPatternInfo(FinderPatternInfo info) throws NotFoundException, FormatException {
        ...
        if (moduleSize < BaseField.BORDER_WIDTH_THIN) {
        ...
        if (provisionalVersion.getAlignmentPatternCenters().length > 0) {
            float correctionToTopLeft = BaseField.BORDER_WIDTH_THIN - (BaseField.BORDER_WIDTH_THICK / ((float) modulesBetweenFPCenters));
    ...
    private static PerspectiveTransform createTransform(ResultPoint topLeft, ResultPoint topRight, ResultPoint bottomLeft, ResultPoint alignmentPattern, int dimension) {
        ...
        if (alignmentPattern != null) {
            ...
            sourceBottomRightX = dimMinusThree - BaseField.BORDER_WIDTH_THICK;
    ...
    protected final float calculateModuleSize(ResultPoint topLeft, ResultPoint topRight, ResultPoint bottomLeft) {
        return (calculateModuleSizeOneWay(topLeft, topRight) + calculateModuleSizeOneWay(topLeft, bottomLeft)) / BaseField.BORDER_WIDTH_MEDIUM;
    }
    ...
    private float sizeOfBlackWhiteBlackRunBothWays(int fromX, int fromY, int toX, int toY) {
        ...
        float scale = BaseField.BORDER_WIDTH_THIN;
        ...
        return (result + sizeOfBlackWhiteBlackRun(fromX, fromY, (int) (((float) fromX) + (((float) (otherToX - fromX)) * scale)), otherToY)) - BaseField.BORDER_WIDTH_THIN;
    }
    ...
    protected final AlignmentPattern findAlignmentInRegion(float overallEstModuleSize, int estAlignmentX, int estAlignmentY, float allowanceFactor) throws NotFoundException {
        ...
        if (((float) (alignmentAreaRightX - alignmentAreaLeftX)) < overallEstModuleSize * BaseField.BORDER_WIDTH_THICK) {
            throw NotFoundException.getNotFoundInstance();
        }
        int alignmentAreaTopY = Math.max(0, estAlignmentY - allowance);
        int alignmentAreaBottomY = Math.min(this.image.getHeight() - 1, estAlignmentY + allowance);
        if (((float) (alignmentAreaBottomY - alignmentAreaTopY)) < overallEstModuleSize * BaseField.BORDER_WIDTH_THICK) {
            throw NotFoundException.getNotFoundInstance();
        }
