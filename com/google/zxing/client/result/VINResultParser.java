...
public final class VINResultParser extends ResultParser {
    ...
    private static int modelYear(char c) {
        ...
        if (c >= '1' && c <= '9') {
            return (c - 49) + FeatureDetector.PYRAMID_FAST;
        }
        if (c >= 'A' && c <= 'D') {
            return (c - 65) + FeatureDetector.PYRAMID_DENSE;
        }
        throw new IllegalArgumentException();
    }
    ...
    private static String countryCode(CharSequence wmi) {
        ...
        switch (c1) {
            ...
            case '9':
                if ((c2 >= 'A' && c2 <= 'E') || (c2 >= PdfWriter.VERSION_1_3 && c2 <= '9')) {
                    return "BR";
                }
            case 'J':
                if (c2 >= 'A' && c2 <= 'T') {
            ...
            case 'W':
                return "DE";
            case 'X':
                if (c2 == '0' || (c2 >= PdfWriter.VERSION_1_3 && c2 <= '9')) {
                    return "RU";
                }
            case 'Z':
                if (c2 >= 'A' && c2 <= 'R') {
