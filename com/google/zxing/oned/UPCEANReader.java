...
public abstract class UPCEANReader extends OneDReader {
    ...
    public Result decodeRow(int rowNumber, BitArray row, int[] startGuardRange, Map<DecodeHintType, ?> hints) throws NotFoundException, ChecksumException, FormatException {
        ...
        if (resultPointCallback != null) {
            resultPointCallback.foundPossibleResultPoint(new ResultPoint(((float) (startGuardRange[0] + startGuardRange[1])) / BaseField.BORDER_WIDTH_MEDIUM, (float) rowNumber));
        }
        ...
        if (resultPointCallback != null) {
            resultPointCallback.foundPossibleResultPoint(new ResultPoint(((float) (endRange[0] + endRange[1])) / BaseField.BORDER_WIDTH_MEDIUM, (float) rowNumber));
        }
        int end = endRange[1];
        ...
        if (resultString.length() < 8) {
            ...
            float left = ((float) (startGuardRange[1] + startGuardRange[0])) / BaseField.BORDER_WIDTH_MEDIUM;
            float right = ((float) (endRange[1] + endRange[0])) / BaseField.BORDER_WIDTH_MEDIUM;
