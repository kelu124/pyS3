...
public final class Detector {
    private static final int[] EXPECTED_CORNER_BITS = new int[]{3808, opencv_videoio.CV_CAP_PROP_XI_GAMMAY, 2107, avutil.AV_CH_LAYOUT_6POINT0};
    ...
    static final class Point {
        private final int f70x;
        private final int f71y;
    ...
    private BitMatrix sampleGrid(BitMatrix image, ResultPoint topLeft, ResultPoint topRight, ResultPoint bottomRight, ResultPoint bottomLeft) throws NotFoundException {
        ...
        float low = (((float) dimension) / BaseField.BORDER_WIDTH_MEDIUM) - ((float) this.nbCenterLayers);
        float high = (((float) dimension) / BaseField.BORDER_WIDTH_MEDIUM) + ((float) this.nbCenterLayers);
    ...
    private static ResultPoint[] expandSquare(ResultPoint[] cornerPoints, float oldSide, float newSide) {
        float ratio = newSide / (BaseField.BORDER_WIDTH_MEDIUM * oldSide);
        float dx = cornerPoints[0].getX() - cornerPoints[2].getX();
        float dy = cornerPoints[0].getY() - cornerPoints[2].getY();
        float centerx = (cornerPoints[0].getX() + cornerPoints[2].getX()) / BaseField.BORDER_WIDTH_MEDIUM;
        float centery = (cornerPoints[0].getY() + cornerPoints[2].getY()) / BaseField.BORDER_WIDTH_MEDIUM;
        ResultPoint result0 = new ResultPoint((ratio * dx) + centerx, (ratio * dy) + centery);
        ResultPoint result2 = new ResultPoint(centerx - (ratio * dx), centery - (ratio * dy));
        dx = cornerPoints[1].getX() - cornerPoints[3].getX();
        dy = cornerPoints[1].getY() - cornerPoints[3].getY();
        centerx = (cornerPoints[1].getX() + cornerPoints[3].getX()) / BaseField.BORDER_WIDTH_MEDIUM;
        centery = (cornerPoints[1].getY() + cornerPoints[3].getY()) / BaseField.BORDER_WIDTH_MEDIUM;
