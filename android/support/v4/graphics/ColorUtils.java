...
public final class ColorUtils {
    ...
    public static void RGBToHSL(@IntRange(from = 0, to = 255) int r, @IntRange(from = 0, to = 255) int g, @IntRange(from = 0, to = 255) int b, @NonNull float[] outHsl) {
        ...
        float l = (max + min) / BaseField.BORDER_WIDTH_MEDIUM;
        if (max == min) {
            s = 0.0f;
            h = 0.0f;
        } else {
            if (max == rf) {
                ...
                h = ((bf - rf) / deltaMaxMin) + BaseField.BORDER_WIDTH_MEDIUM;
            } else {
                h = ((rf - gf) / deltaMaxMin) + 4.0f;
            }
            s = deltaMaxMin / (BaseField.BORDER_WIDTH_THIN - Math.abs((BaseField.BORDER_WIDTH_MEDIUM * l) - BaseField.BORDER_WIDTH_THIN));
        }
        h = (60.0f * h) % 360.0f;
        if (h < 0.0f) {
            h += 360.0f;
        ...
        outHsl[1] = constrain(s, 0.0f, (float) BaseField.BORDER_WIDTH_THIN);
        outHsl[2] = constrain(l, 0.0f, (float) BaseField.BORDER_WIDTH_THIN);
    }
    ...
    public static int HSLToColor(@NonNull float[] hsl) {
        ...
        float s = hsl[1];
        float l = hsl[2];
        float c = (BaseField.BORDER_WIDTH_THIN - Math.abs((BaseField.BORDER_WIDTH_MEDIUM * l) - BaseField.BORDER_WIDTH_THIN)) * s;
        float m = l - (0.5f * c);
        float x = c * (BaseField.BORDER_WIDTH_THIN - Math.abs(((h / 60.0f) % BaseField.BORDER_WIDTH_MEDIUM) - BaseField.BORDER_WIDTH_THIN));
        int r = 0;
        int g = 0;
        int b = 0;
        switch (((int) h) / 60) {
