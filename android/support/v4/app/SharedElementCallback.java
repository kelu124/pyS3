...
public abstract class SharedElementCallback {
    ...
    public Parcelable onCaptureSharedElementSnapshot(View sharedElement, Matrix viewToGlobalMatrix, RectF screenBounds) {
        ...
        if (bitmapWidth > 0 && bitmapHeight > 0) {
            float scale = Math.min(BaseField.BORDER_WIDTH_THIN, ((float) MAX_IMAGE_SIZE) / ((float) (bitmapWidth * bitmapHeight)));
    ...
    private static Bitmap createDrawableBitmap(Drawable drawable) {
        ...
        float scale = Math.min(BaseField.BORDER_WIDTH_THIN, ((float) MAX_IMAGE_SIZE) / ((float) (width * height)));
        if ((drawable instanceof BitmapDrawable) && scale == BaseField.BORDER_WIDTH_THIN) {
