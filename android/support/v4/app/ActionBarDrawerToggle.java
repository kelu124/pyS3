...
public class ActionBarDrawerToggle implements DrawerListener {
    ...
    public void syncState() {
        if (this.mDrawerLayout.isDrawerOpen((int) GravityCompat.START)) {
            this.mSlider.setPosition(BaseField.BORDER_WIDTH_THIN);
        } else {
    ...
    public void onDrawerSlide(View drawerView, float slideOffset) {
        ...
        if (slideOffset > 0.5f) {
            glyphOffset = Math.max(glyphOffset, Math.max(0.0f, slideOffset - 0.5f) * BaseField.BORDER_WIDTH_MEDIUM);
        } else {
            glyphOffset = Math.min(glyphOffset, slideOffset * BaseField.BORDER_WIDTH_MEDIUM);
        }
