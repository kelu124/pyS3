...
public class ViewPager extends ViewGroup {
    ...
    private static final Comparator<ItemInfo> COMPARATOR = new C02171();
    ...
    private static final Interpolator sInterpolator = new C02182();
    ...
    private final Runnable mEndScrollRunnable = new C02193();
    ...
    static class C02171 implements Comparator<ItemInfo> {
        C02171() {
        }
    ...
    static class C02182 implements Interpolator {
        C02182() {
        }
    ...
    class C02204 implements OnApplyWindowInsetsListener {
        private final Rect mTempRect = new Rect();
    ...
    void initViewPager() {
        ...
        this.mCloseEnough = (int) (BaseField.BORDER_WIDTH_MEDIUM * density);
        this.mDefaultGutterSize = (int) (16.0f * density);
        ViewCompat.setAccessibilityDelegate(this, new MyAccessibilityDelegate());
        if (ViewCompat.getImportantForAccessibility(this) == 0) {
            ViewCompat.setImportantForAccessibility(this, 1);
        }
        ViewCompat.setOnApplyWindowInsetsListener(this, new C02204());
    }
    ...
    void smoothScrollTo(int x, int y, int velocity) {
        ...
        float distance = ((float) halfWidth) + (((float) halfWidth) * distanceInfluenceForSnapDuration(Math.min(BaseField.BORDER_WIDTH_THIN, (BaseField.BORDER_WIDTH_THIN * ((float) Math.abs(dx))) / ((float) width))));
        velocity = Math.abs(velocity);
        if (velocity > 0) {
            duration = Math.round(1000.0f * Math.abs(distance / ((float) velocity))) * 4;
        } else {
            duration = (int) ((BaseField.BORDER_WIDTH_THIN + (((float) Math.abs(dx)) / (((float) this.mPageMargin) + (((float) width) * this.mAdapter.getPageWidth(this.mCurItem))))) * 100.0f);
        }
                ...
                if (ii.position >= this.mCurItem) {
                    ...
                    if (curItem != null) {
                        ...
                        if (clientWidth > 0) {
                            ...
                            leftWidthNeeded = (BaseField.BORDER_WIDTH_MEDIUM - curItem.widthFactor) + (((float) getPaddingLeft()) / ((float) clientWidth));
                        }
                        pos = this.mCurItem - 1;
                        while (pos >= 0) {
                        ...
                        itemIndex = curIndex + 1;
                        if (extraWidthRight < BaseField.BORDER_WIDTH_MEDIUM) {
                            ii = itemIndex >= this.mItems.size() ? (ItemInfo) this.mItems.get(itemIndex) : null;
                            if (clientWidth > 0) {
                                rightWidthNeeded = 0.0f;
                            } else {
                                rightWidthNeeded = (((float) getPaddingRight()) / ((float) clientWidth)) + BaseField.BORDER_WIDTH_MEDIUM;
                            }
                            pos = this.mCurItem + 1;
                            while (pos < N) {
    ...
    private void calculatePageOffsets(ItemInfo curItem, int curIndex, ItemInfo oldCurInfo) {
        ...
        this.mLastOffset = curItem.position == N + -1 ? (curItem.offset + curItem.widthFactor) - BaseField.BORDER_WIDTH_THIN : AutoScrollHelper.NO_MAX;
        int i = curIndex - 1;
        while (i >= 0) {
        ...
        while (i < itemCount) {
            ...
            if (ii.position == N - 1) {
                this.mLastOffset = (ii.widthFactor + offset) - BaseField.BORDER_WIDTH_THIN;
            }
            ii.offset = offset;
            offset += ii.widthFactor + marginOffset;
            i++;
    ...
    public void draw(Canvas canvas) {
        ...
        if (overScrollMode == 0 || (overScrollMode == 1 && this.mAdapter != null && this.mAdapter.getCount() > 1)) {
            ...
            if (!this.mRightEdge.isFinished()) {
                ...
                height = (getHeight() - getPaddingTop()) - getPaddingBottom();
                canvas.rotate(90.0f);
                canvas.translate((float) (-getPaddingTop()), (-(this.mLastOffset + BaseField.BORDER_WIDTH_THIN)) * ((float) width));
