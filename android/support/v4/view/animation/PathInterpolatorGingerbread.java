...
class PathInterpolatorGingerbread implements Interpolator {
    ...
    public float getInterpolation(float t) {
        ...
        if (t >= BaseField.BORDER_WIDTH_THIN) {
    ...
    private static Path createQuad(float controlX, float controlY) {
        ...
        path.quadTo(controlX, controlY, BaseField.BORDER_WIDTH_THIN, BaseField.BORDER_WIDTH_THIN);
        return path;
    }
    ...
    private static Path createCubic(float controlX1, float controlY1, float controlX2, float controlY2) {
        Path path = new Path();
        path.moveTo(0.0f, 0.0f);
        path.cubicTo(controlX1, controlY1, controlX2, controlY2, BaseField.BORDER_WIDTH_THIN, BaseField.BORDER_WIDTH_THIN);
        return path;
    }
}
