...
public class ViewDragHelper {
    ...
    private static final Interpolator sInterpolator = new C02551();
    ...
    private final Runnable mSetIdleRunnable = new C02562();
    ...
    static class C02551 implements Interpolator {
        C02551() {
        }
    ...
    public static ViewDragHelper create(ViewGroup forParent, float sensitivity, Callback cb) {
        ...
        helper.mTouchSlop = (int) (((float) helper.mTouchSlop) * (BaseField.BORDER_WIDTH_THIN / sensitivity));
        return helper;
    }
    ...
    private int computeAxisDuration(int delta, int velocity, int motionRange) {
        ...
        float distance = ((float) halfWidth) + (((float) halfWidth) * distanceInfluenceForSnapDuration(Math.min(BaseField.BORDER_WIDTH_THIN, ((float) Math.abs(delta)) / ((float) width))));
        velocity = Math.abs(velocity);
        if (velocity > 0) {
            duration = Math.round(1000.0f * Math.abs(distance / ((float) velocity))) * 4;
        } else {
            duration = (int) (((((float) Math.abs(delta)) / ((float) motionRange)) + BaseField.BORDER_WIDTH_THIN) * 256.0f);
        }
