...
public class ActionBarDrawerToggle implements DrawerListener {
    ...
    ActionBarDrawerToggle(Activity activity, Toolbar toolbar, DrawerLayout drawerLayout, DrawerArrowDrawable slider, @StringRes int openDrawerContentDescRes, @StringRes int closeDrawerContentDescRes) {
        ...
        if (toolbar != null) {
            ...
            toolbar.setNavigationOnClickListener(new C02571());
    ...
    public void syncState() {
        if (this.mDrawerLayout.isDrawerOpen((int) GravityCompat.START)) {
            setPosition(BaseField.BORDER_WIDTH_THIN);
        } else {
            setPosition(0.0f);
        }
    ...
    public void onDrawerSlide(View drawerView, float slideOffset) {
        if (this.mDrawerSlideAnimationEnabled) {
            setPosition(Math.min(BaseField.BORDER_WIDTH_THIN, Math.max(0.0f, slideOffset)));
        } else {
            setPosition(0.0f);
        }
    }
    ...
    private void setPosition(float position) {
        if (position == BaseField.BORDER_WIDTH_THIN) {
