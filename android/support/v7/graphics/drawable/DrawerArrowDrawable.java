...
public class DrawerArrowDrawable extends Drawable {
    ...
    public DrawerArrowDrawable(Context context) {
        ...
        TypedArray a = context.getTheme().obtainStyledAttributes(null, C0284R.styleable.DrawerArrowToggle, C0284R.attr.drawerArrowStyle, C0284R.style.Base_Widget_AppCompat_DrawerArrowToggle);
        setColor(a.getColor(C0284R.styleable.DrawerArrowToggle_color, 0));
        setBarThickness(a.getDimension(C0284R.styleable.DrawerArrowToggle_thickness, 0.0f));
        setSpinEnabled(a.getBoolean(C0284R.styleable.DrawerArrowToggle_spinBars, true));
        setGapSize((float) Math.round(a.getDimension(C0284R.styleable.DrawerArrowToggle_gapBetweenBars, 0.0f)));
        this.mSize = a.getDimensionPixelSize(C0284R.styleable.DrawerArrowToggle_drawableSize, 0);
        this.mBarLength = (float) Math.round(a.getDimension(C0284R.styleable.DrawerArrowToggle_barLength, 0.0f));
        this.mArrowHeadLength = (float) Math.round(a.getDimension(C0284R.styleable.DrawerArrowToggle_arrowHeadLength, 0.0f));
        this.mArrowShaftLength = a.getDimension(C0284R.styleable.DrawerArrowToggle_arrowShaftLength, 0.0f);
        a.recycle();
    }
    ...
    public void setBarThickness(float width) {
        if (this.mPaint.getStrokeWidth() != width) {
            ...
            this.mMaxCutForBarSize = (float) (((double) (width / BaseField.BORDER_WIDTH_MEDIUM)) * Math.cos((double) ARROW_HEAD_ANGLE));
            invalidateSelf();
        }
    }
    ...
    public void draw(Canvas canvas) {
        ...
        float arrowHeadBarLength = lerp(this.mBarLength, (float) Math.sqrt((double) ((this.mArrowHeadLength * this.mArrowHeadLength) * BaseField.BORDER_WIDTH_MEDIUM)), this.mProgress);
        ...
        float arrowEdge = (-arrowShaftLength) / BaseField.BORDER_WIDTH_MEDIUM;
        this.mPath.moveTo(arrowEdge + arrowShaftCut, 0.0f);
        this.mPath.rLineTo(arrowShaftLength - (BaseField.BORDER_WIDTH_MEDIUM * arrowShaftCut), 0.0f);
        ...
        canvas.translate((float) bounds.centerX(), (float) (((double) ((float) ((((int) ((((float) bounds.height()) - (BaseField.BORDER_WIDTH_THICK * barThickness)) - (this.mBarGap * BaseField.BORDER_WIDTH_MEDIUM))) / 4) * 2))) + ((((double) barThickness) * 1.5d) + ((double) this.mBarGap))));
        if (this.mSpin) {
