...
public class DiffUtil {
    private static final Comparator<Snake> SNAKE_COMPARATOR = new C02891();

    static class C02891 implements Comparator<Snake> {
        C02891() {
        }

        public int compare(Snake o1, Snake o2) {
            int cmpX = o1.f15x - o2.f15x;
            return cmpX == 0 ? o1.f16y - o2.f16y : cmpX;
        }
    }
    ...
    public static class DiffResult {
        ...
        private void addRootSnake() {
            ...
            if (firstSnake == null || firstSnake.f15x != 0 || firstSnake.f16y != 0) {
        ...
        private void findMatchingItems() {
            ...
            for (int i = this.mSnakes.size() - 1; i >= 0; i--) {
                ...
                int endX = snake.f15x + snake.size;
                int endY = snake.f16y + snake.size;
                ...
                for (int j = 0; j < snake.size; j++) {
                    int oldItemPos = snake.f15x + j;
                    int newItemPos = snake.f16y + j;
        ...
        public void dispatchUpdatesTo(ListUpdateCallback updateCallback) {
            ...
            for (int snakeIndex = this.mSnakes.size() - 1; snakeIndex >= 0; snakeIndex--) {
                ...
                int endX = snake.f15x + snakeSize;
                int endY = snake.f16y + snakeSize;
                if (endX < posOld) {
                ...
                for (int i = snakeSize - 1; i >= 0; i--) {
                    if ((this.mOldItemStatuses[snake.f15x + i] & 31) == 2) {
                        batchingCallback.onChanged(snake.f15x + i, 1, this.mCallback.getChangePayload(snake.f15x + i, snake.f16y + i));
                    }
                }
                posOld = snake.f15x;
                posNew = snake.f16y;
    ...
    public static DiffResult calculateDiff(Callback cb, boolean detectMoves) {
        ...
        while (!stack.isEmpty()) {
            ...
            if (snake != null) {
                ...
                snake.f15x += range.oldListStart;
                snake.f16y += range.newListStart;
                ...
                if (snake.reverse) {
                    left.oldListEnd = snake.f15x;
                    left.newListEnd = snake.f16y;
                } else if (snake.removal) {
                    left.oldListEnd = snake.f15x - 1;
                    left.newListEnd = snake.f16y;
                } else {
                    left.oldListEnd = snake.f15x;
                    left.newListEnd = snake.f16y - 1;
                }
                stack.add(left);
                Range right = range;
                if (!snake.reverse) {
                    right.oldListStart = snake.f15x + snake.size;
                    right.newListStart = snake.f16y + snake.size;
                } else if (snake.removal) {
                    right.oldListStart = (snake.f15x + snake.size) + 1;
                    right.newListStart = snake.f16y + snake.size;
                } else {
                    right.oldListStart = snake.f15x + snake.size;
                    right.newListStart = (snake.f16y + snake.size) + 1;
                }
                stack.add(right);
            } else {
                rangePool.add(range);
        ...
        while (d <= dLimit) {
            ...
            while (k <= d) {
                ...
                if (!checkInFwd || k < (delta - d) + 1 || k > (delta + d) - 1 || forward[kOffset + k] < backward[kOffset + k]) {
                    ...
                    outSnake.f15x = backward[kOffset + k];
                    outSnake.f16y = outSnake.f15x - k;
            ...
            while (k <= d) {
                ...
                if (checkInFwd || k + delta < (-d) || k + delta > d || forward[kOffset + backwardK] < backward[kOffset + backwardK]) {
                    ...
                    outSnake = new Snake();
                    outSnake.f15x = backward[kOffset + backwardK];
                    outSnake.f16y = outSnake.f15x - backwardK;
