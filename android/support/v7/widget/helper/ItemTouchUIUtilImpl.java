...
class ItemTouchUIUtilImpl {
    ...
    static class Lollipop extends Honeycomb {
        ...
        public void onDraw(Canvas c, RecyclerView recyclerView, View view, float dX, float dY, int actionState, boolean isCurrentlyActive) {
            if (isCurrentlyActive && view.getTag(C0286R.id.item_touch_helper_previous_elevation) == null) {
                Float originalElevation = Float.valueOf(ViewCompat.getElevation(view));
                ViewCompat.setElevation(view, BaseField.BORDER_WIDTH_THIN + findMaxElevation(recyclerView, view));
                view.setTag(C0286R.id.item_touch_helper_previous_elevation, originalElevation);
            }
        ...
        public void clearView(View view) {
            Object tag = view.getTag(C0286R.id.item_touch_helper_previous_elevation);
            if (tag != null && (tag instanceof Float)) {
                ViewCompat.setElevation(view, ((Float) tag).floatValue());
            }
            view.setTag(C0286R.id.item_touch_helper_previous_elevation, null);
