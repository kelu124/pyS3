...
public class SnackbarContentLayout extends LinearLayout implements ContentViewCallback {
    ...
    public void animateContentIn(int delay, int duration) {
        ...
        ViewCompat.animate(this.mMessageView).alpha(BaseField.BORDER_WIDTH_THIN).setDuration((long) duration).setStartDelay((long) delay).start();
        if (this.mActionView.getVisibility() == 0) {
            ViewCompat.setAlpha(this.mActionView, 0.0f);
            ViewCompat.animate(this.mActionView).alpha(BaseField.BORDER_WIDTH_THIN).setDuration((long) duration).setStartDelay((long) delay).start();
        }
    }

    public void animateContentOut(int delay, int duration) {
        ViewCompat.setAlpha(this.mMessageView, BaseField.BORDER_WIDTH_THIN);
        ViewCompat.animate(this.mMessageView).alpha(0.0f).setDuration((long) duration).setStartDelay((long) delay).start();
        if (this.mActionView.getVisibility() == 0) {
            ViewCompat.setAlpha(this.mActionView, BaseField.BORDER_WIDTH_THIN);
