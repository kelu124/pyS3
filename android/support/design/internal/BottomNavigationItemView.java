...
public class BottomNavigationItemView extends FrameLayout implements ItemView {
    ...
    public BottomNavigationItemView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        int inactiveLabelSize = res.getDimensionPixelSize(C0010R.dimen.design_bottom_navigation_text_size);
        int activeLabelSize = res.getDimensionPixelSize(C0010R.dimen.design_bottom_navigation_active_text_size);
        this.mDefaultMargin = res.getDimensionPixelSize(C0010R.dimen.design_bottom_navigation_margin);
        this.mShiftAmount = inactiveLabelSize - activeLabelSize;
        this.mScaleUpFactor = (((float) activeLabelSize) * BaseField.BORDER_WIDTH_THIN) / ((float) inactiveLabelSize);
        this.mScaleDownFactor = (((float) inactiveLabelSize) * BaseField.BORDER_WIDTH_THIN) / ((float) activeLabelSize);
        LayoutInflater.from(context).inflate(C0010R.layout.design_bottom_navigation_item, this, true);
        setBackgroundResource(C0010R.drawable.design_bottom_navigation_item_background);
        this.mIcon = (ImageView) findViewById(C0010R.id.icon);
        this.mSmallLabel = (TextView) findViewById(C0010R.id.smallLabel);
        this.mLargeLabel = (TextView) findViewById(C0010R.id.largeLabel);
    }
    ...
    public void setChecked(boolean checked) {
        ...
        if (this.mShiftingMode) {
            if (checked) {
                ...
                ViewCompat.setScaleX(this.mLargeLabel, BaseField.BORDER_WIDTH_THIN);
                ViewCompat.setScaleY(this.mLargeLabel, BaseField.BORDER_WIDTH_THIN);
            } else {
                iconParams = (LayoutParams) this.mIcon.getLayoutParams();
            ...
            ViewCompat.setScaleX(this.mSmallLabel, BaseField.BORDER_WIDTH_THIN);
            ViewCompat.setScaleY(this.mSmallLabel, BaseField.BORDER_WIDTH_THIN);
        }
        refreshDrawableState();
    }
