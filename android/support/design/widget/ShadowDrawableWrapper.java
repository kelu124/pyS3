...
class ShadowDrawableWrapper extends DrawableWrapper {
    ...
    private void drawShadow(Canvas canvas) {
        ...
        boolean drawHorizontalEdges = this.mContentBounds.width() - (BaseField.BORDER_WIDTH_MEDIUM * shadowOffset) > 0.0f;
        boolean drawVerticalEdges = this.mContentBounds.height() - (BaseField.BORDER_WIDTH_MEDIUM * shadowOffset) > 0.0f;
        ...
        if (drawHorizontalEdges) {
            ...
            canvas.drawRect(0.0f, edgeShadowTop, this.mContentBounds.width() - (BaseField.BORDER_WIDTH_MEDIUM * shadowOffset), -this.mCornerRadius, this.mEdgeShadowPaint);
        }
        ...
        if (drawHorizontalEdges) {
            canvas.scale(1.0f / shadowScaleHorizontal, 1.0f);
            canvas.drawRect(0.0f, edgeShadowTop, this.mContentBounds.width() - (BaseField.BORDER_WIDTH_MEDIUM * shadowOffset), this.mShadowSize + (-this.mCornerRadius), this.mEdgeShadowPaint);
        }
        canvas.restoreToCount(saved);
        saved = canvas.save();
        ...
        if (drawVerticalEdges) {
            ...
            canvas.drawRect(0.0f, edgeShadowTop, this.mContentBounds.height() - (BaseField.BORDER_WIDTH_MEDIUM * shadowOffset), -this.mCornerRadius, this.mEdgeShadowPaint);
        }
        canvas.restoreToCount(saved);
        saved = canvas.save();
    ...
    private void buildShadowCorners() {
        ...
        if (shadowRadius > 0.0f) {
            ...
            float midRatio = startRatio + ((1.0f - startRatio) / BaseField.BORDER_WIDTH_MEDIUM);
    ...
    public float getMinWidth() {
        return (this.mRawMaxShadowSize * BaseField.BORDER_WIDTH_MEDIUM) + (BaseField.BORDER_WIDTH_MEDIUM * Math.max(this.mRawMaxShadowSize, this.mCornerRadius + (this.mRawMaxShadowSize / BaseField.BORDER_WIDTH_MEDIUM)));
    }

    public float getMinHeight() {
        return ((this.mRawMaxShadowSize * SHADOW_MULTIPLIER) * BaseField.BORDER_WIDTH_MEDIUM) + (BaseField.BORDER_WIDTH_MEDIUM * Math.max(this.mRawMaxShadowSize, this.mCornerRadius + ((this.mRawMaxShadowSize * SHADOW_MULTIPLIER) / BaseField.BORDER_WIDTH_MEDIUM)));
    }
}
