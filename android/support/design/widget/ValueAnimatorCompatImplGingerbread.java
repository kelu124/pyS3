...
class ValueAnimatorCompatImplGingerbread extends Impl {
    ...
    private final Runnable mRunnable = new C00671();
    ...
    public void end() {
        if (this.mIsRunning) {
            this.mIsRunning = false;
            sHandler.removeCallbacks(this.mRunnable);
            this.mAnimatedFraction = BaseField.BORDER_WIDTH_THIN;
            dispatchAnimationUpdate();
            dispatchAnimationEnd();
        }
    }
    ...
    final void update() {
        if (this.mIsRunning) {
            float linearFraction = MathUtils.constrain(((float) (SystemClock.uptimeMillis() - this.mStartTime)) / ((float) this.mDuration), 0.0f, (float) BaseField.BORDER_WIDTH_THIN);
            if (this.mInterpolator != null) {
