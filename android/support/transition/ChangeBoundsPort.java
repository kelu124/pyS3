...
class ChangeBoundsPort extends TransitionPort {
    ...
    class C00811 extends TransitionListenerAdapter {
    ...
    class C00822 extends TransitionListenerAdapter {
        boolean mCanceled = false;
    ...
    class C00833 extends AnimatorListenerAdapter {
        C00833() {
        }
            ...
            if (numChanges > 0) {
                ...
                if (this.mResizeClip) {
                    ...
                    if (view.getParent() instanceof ViewGroup) {
                        ...
                        addListener(new C00822());
                    }
                    anim.addListener(new C00833());
                    return anim;
                }
                pvh = new PropertyValuesHolder[numChanges];
                if (startLeft != endLeft) {
                ...
                if (startLeft != endLeft) {
                    pvhIndex = 0 + 1;
                    pvh[0] = PropertyValuesHolder.ofInt(HtmlTags.ALIGN_LEFT, new int[]{startLeft, endLeft});
                } else {
                    pvhIndex = 0;
                }
                if (startTop != endTop) {
                    i = pvhIndex + 1;
                    pvh[pvhIndex] = PropertyValuesHolder.ofInt(HtmlTags.ALIGN_TOP, new int[]{startTop, endTop});
                    pvhIndex = i;
                }
                if (startRight != endRight) {
                    i = pvhIndex + 1;
                    pvh[pvhIndex] = PropertyValuesHolder.ofInt(HtmlTags.ALIGN_RIGHT, new int[]{startRight, endRight});
                    pvhIndex = i;
                }
                if (startBottom != endBottom) {
                    i = pvhIndex + 1;
                    pvh[pvhIndex] = PropertyValuesHolder.ofInt(HtmlTags.ALIGN_BOTTOM, new int[]{startBottom, endBottom});
                } else {
                    i = pvhIndex;
                }
                anim = ObjectAnimator.ofPropertyValuesHolder(view, pvh);
