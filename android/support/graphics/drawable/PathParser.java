...
class PathParser {
    ...
    public static class PathDataNode {
        ...
        public void interpolatePathDataNode(PathDataNode nodeFrom, PathDataNode nodeTo, float fraction) {
            for (int i = 0; i < nodeFrom.params.length; i++) {
                this.params[i] = (nodeFrom.params[i] * (BaseField.BORDER_WIDTH_THIN - fraction)) + (nodeTo.params[i] * fraction);
            }
        }
                ...
                switch (cmd) {
                    ...
                    case 'S':
                        ...
                        if (previousCmd == 'c' || previousCmd == 's' || previousCmd == 'C' || previousCmd == 'S') {
                            reflectiveCtrlPointX = (BaseField.BORDER_WIDTH_MEDIUM * currentX) - ctrlPointX;
                            reflectiveCtrlPointY = (BaseField.BORDER_WIDTH_MEDIUM * currentY) - ctrlPointY;
                        }
    ...
    private static int nextStart(String s, int end) {
        while (end < s.length()) {
            char c = s.charAt(end);
            if (((c - 65) * (c - 90) <= 0 || (c - 97) * (c - 122) <= 0) && c != Barcode128.CODE_BC_TO_A && c != 'E') {
                break;
            }
            end++;
        }
