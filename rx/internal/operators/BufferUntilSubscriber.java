...
public final class BufferUntilSubscriber<T> extends Subject<T, T> {
    private static final Observer EMPTY_OBSERVER = new C13341();
    private boolean forward = false;
    final State<T> state;

    static class C13341 implements Observer {
        C13341() {
        }
    ...
    static final class OnSubscribeAction<T> implements Observable$OnSubscribe<T> {
        ...
        class C13351 implements Action0 {
            C13351() {
            }
        ...
        public void call(Subscriber<? super T> s) {
            if (this.state.casObserverRef(null, s)) {
                s.add(Subscriptions.create(new C13351()));
                boolean win = false;
