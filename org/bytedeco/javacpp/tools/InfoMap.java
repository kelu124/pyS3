...
public class InfoMap extends HashMap<String, List<Info>> {
    ...
    String normalize(String name, boolean unconst, boolean untemplate) {
        if (!(name == null || name.length() == 0)) {
            if (!name.startsWith("basic/")) {
                ...
                while (i < n) {
                    int j;
                    if (tokens[i].match(new Object[]{Token.CONST, Token.CONSTEXPR})) {
                        foundConst = true;
                        for (j = i + 1; j < n; j++) {
                            tokens[j - 1] = tokens[j];
                        }
                        ...
                        if (tokens[i].match(new Object[]{Token.CLASS, Token.STRUCT, Token.UNION})) {
                ...
                if (simpleType) {
                    ...
                    i = 0;
                    while (i < n) {
                        if (tokens[i].match(new Object[]{Character.valueOf('<')})) {
                            if (count == 0) {
                                template = i;
                            }
                            count++;
    ...
    public List<Info> get(String cppName, boolean partial) {
        List<Info> list = (List) super.get(normalize(cppName, false, false));
        if (list == null) {
            list = (List) super.get(normalize(cppName, true, false));
        }
        if (list == null && partial) {
            list = (List) super.get(normalize(cppName, true, true));
        }
        if (list == null) {
            list = new ArrayList();
        }
        ...
        List<Info> l = this.parent.get(cppName, partial);
        if (l == null || l.size() <= 0) {
            return list;
        }
        List<Info> infoList = new ArrayList(list);
        infoList.addAll(l);
        return infoList;
    }
    ...
    public Info get(int index, String cppName, boolean partial) {
        ...
        return infoList.size() > 0 ? (Info) infoList.get(index) : null;
    }
    ...
    public Info getFirst(String cppName, boolean partial) {
        List<Info> infoList = get(cppName, partial);
        return infoList.size() > 0 ? (Info) infoList.get(0) : null;
    }
